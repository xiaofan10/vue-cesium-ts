...
class Math3d {
   ...
   * @private
   */
  raisePositionsToHeight(positions, options, extrude) {
    if (this._viewer) {
   ...
   * @private
   */
  computeEllipseEdgePositions(options) {
    if (this._viewer) {
   ...
   * @private
   */
  computeSectorEdgePositions(options) {
    if (this._viewer) {
      var unitPosScratch = new Cesium.Cartesian3()
      var eastVecScratch = new Cesium.Cartesian3()
      var northVecScratch = new Cesium.Cartesian3()
  ...
  _checkLonDegree(value) {
    if (value > 180 || value < -180) {
      return false
    }
    return true
  }
  _checkLatDegree(value) {
  ...
  /*
        ...
        return [[lon,lat],...]
        */
  computeInterpolateLineLonlat(start, end) {
    if (!start || !end) {
      return null
    }
  ...
  /*
        ...
        return [Cartographic,...]
        */
  computeInterpolateLineCartographic(start, end, _Delta) {
    if (!start || !end) {
      return null
    }
  ...
  /*
        ...
        return [Cartographic,...]
        */
  computeInterpolateLineHeightCartographic(start, end) {
    if (!start || !end) {
      return null
    }
    if (!start.longitude || !start.latitude || !end.longitude || !end.latitude) {
  ...
  /*
        ...
        return [Cartographic,...]
        */
  computeInterpolate2IndexLineHeightCartographic(start, end, num, curIndex) {
    if (!start || !end) {
      return null
    }
    if (!start.longitude || !start.latitude || !end.longitude || !end.latitude) {
  ...
  /*
        ...
        return Cartographic
        */
  computeInterpolateIndexLineHeightCartographic(start, end, num, index) {
    if (!start || !end) {
      return null
    }
    if (!start.longitude || !start.latitude || !end.longitude || !end.latitude) {
